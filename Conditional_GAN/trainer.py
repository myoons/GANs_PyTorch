...
class Trainer:
    ...
    def train(self):
        ...
        def run_epoch(epoch, fake_y):
            ...
            for step, (real_images, real_y) in pbar:
                batch_size = real_images.size(0)

                real_images = real_images.to(self.device)
                real_y = real_y.long().to(self.device)
                ...
                real_outputs = discriminator(real_images, real_y).view(-1)
                d_scores_real.append(real_outputs.mean().item())
                real_loss_d = criterion(real_outputs, real_labels)

                z = torch.randn(batch_size, config.latent_size).to(self.device)

                fake_images = generator(z, fake_y)
                fake_outputs = discriminator(fake_images, fake_y).view(-1)
            ...
            if epoch == 0:
                save_image(denormalize(real_images), os.path.join(f"{config.ckpt_path}", "images", "real_images.png"))

            save_image(denormalize(fake_images),
                       os.path.join(f"{config.ckpt_path}", "images", f"fake_images_{epoch + 1}.png"))

            if (epoch + 1) % 20 == 0:
                self.save_checkpoint(epoch + 1)

        ones = torch.ones(config.batch_size // 10)

        fake_y = torch.cat([ones * 0, ones * 1, ones * 2, ones * 3, ones * 4,
                            ones * 5, ones * 6, ones * 7, ones * 8, ones * 9], dim=0).long().to(self.device)
        for epc in range(config.epochs):
            run_epoch(epc, fake_y)
